...
public:
  /* x > y > z */
  const int32_t x_var = 2;
  const int32_t y_var = 1;
  const int32_t z_var = 0;
...
protected:
  virtual void SetUp() {
    ...
    f2 = Polynomial(unit, z, yz, x, y, xyz, xz, xy);
    g = Polynomial(yz, y);
    h = Polynomial(x, unit);
...
TEST_F(PolynomialTest, AddTest) {
  ...
  ASSERT_TRUE(f + u == Polynomial(2 * xyz, 2 * xy, 2 * xz, 2 * yz, 3 * x, 2 * y,
                                  2 * z, 2 * unit));
